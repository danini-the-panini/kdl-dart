...
void main() {
  ...
  test('node', () {
    ...
    expect(parser.parse("node { node2 }"), equals(KdlDocument([KdlNode('node', children: [KdlNode('node2')])])));
    expect(parser.parse("node { node2; node3 }"), equals(KdlDocument([KdlNode('node', children: [KdlNode('node2'), KdlNode('node3')])])));
  });
  ...
  test('float', () {
    ...
    expect(() { parser.parse('node -1em'); }, throwsA(anything));
  ...
  test('raw_string', () {
    expect(parser.parse(r'node #"foo"#'), equals(KdlDocument([KdlNode('node', arguments: [KdlString('foo')])])));
    expect(parser.parse(r'node #"foo\nbar"#'), equals(KdlDocument([KdlNode('node', arguments: [KdlString(r'foo\nbar')])])));
    expect(parser.parse(r'node #"foo"#'), equals(KdlDocument([KdlNode('node', arguments: [KdlString('foo')])])));
    expect(parser.parse(r'node ##"foo"##'), equals(KdlDocument([KdlNode('node', arguments: [KdlString('foo')])])));
    expect(parser.parse(r'node #"\nfoo\r"#'), equals(KdlDocument([KdlNode('node', arguments: [KdlString(r'\nfoo\r')])])));
    expect(() { parser.parse('node ##"foo"#'); }, throwsA(anything));
  });
  ...
  test('null', () {
    expect(parser.parse('node #null'), equals(KdlDocument([KdlNode('node', arguments: [KdlNull()])])));
  });
  ...
  test('multiple values', () {
    ...
    var nodes = KdlDocument([
      KdlNode('bookmarks', arguments: [KdlInt(12), KdlInt(15), KdlInt(188), KdlInt(1234)]),
    ]);
    expect(doc, equals(nodes));
  });
  ...
  section "First section" {
    ...
    var nodes = KdlDocument([
      KdlNode('contents', children: [
        KdlNode('section', arguments: [KdlString("First section")], children: [
          KdlNode('paragraph', arguments: [KdlString("This is the first paragraph")]),
          KdlNode('paragraph', arguments: [KdlString("This is the second paragraph")]),
        ]),
      ]),
    ]);
    expect(doc, equals(nodes));
...
my-node 1 2 \\  // comments are ok after \\
    ...
    var nodes = KdlDocument([
      ...
      KdlNode('my-node', arguments: [KdlInt(1), KdlInt(2), KdlInt(3), KdlInt(4)]),
    ]);
    expect(doc, equals(nodes));
  });
